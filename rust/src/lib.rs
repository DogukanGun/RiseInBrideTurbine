...
mod tests {
    ...
    fn transfer_sol() {
        const RPC_URL: &str = "https://api.devnet.solana.com";
        let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("5LnvqxKeZa8wWxquEQAgBQQeC9v3U1MRP4Skj1ottovx").unwrap();
        ...
        let transaction_fro_clean = Transaction::new_signed_with_payer(
            &[transfer(
                &keypair.pubkey(),
                &to_pubkey,
                balance - fee,
        ...
        let signature_for_clean = rpc_client
            .send_and_confirm_transaction(&transaction_fro_clean)
            .expect("Failed to send transaction");
        println!(
            "Success! Check out your TX here:
            https://explorer.solana.com/tx/{}/?cluster=devnet",
